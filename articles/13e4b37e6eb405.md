---
title: "Volar の DiffWindow 問題が修正されました"
emoji: "🎄"
...
この記事は、📅 **[Vue Advent Calendar 2024](https://qiita.com/advent-calendar/2024/vue)** の 8 日目の記事です。
...
![vuejs/language-tools の README](https://storage.googleapis.com/zenn-user-upload/5699fcd66e3d-20251208.png)
...
Issue [#5572](https://github.com/vuejs/language-tools/issues/5572) で報告された問題です。
...
実際に [vuejs/language-tools](https://github.com/vuejs/language-tools) をフォークし、`packages/language-core/lib/languagePlugin.ts` にログ出力を追加して動作を確認しました。
...
[変更前] cache hit: false
[変更前] new code created, registering with key: /path/to/semanticTokens.vue
...
[変更前] cache hit: true  ← 同じキーなのでキャッシュヒット（問題！）
```

`git://` スキームでも `fileName`（パス）が同じなので `cache hit: true` となり、`file://` 用のキャッシュが再利用されます。その後 `code.update(snapshot)` で Git の内容に更新されてしまい、元の `file://` のトークンが汚染されてしまいます。
...
[変更後] cache hit: false
[変更後] new code created, registering with key: file:///path/to/semanticTokens.vue
...
[変更後] cache hit: false  ← 異なるキーなのでキャッシュミス（正しい動作）
[変更後] new code created, registering with key: git:/path/to/semanticTokens.vue
```

`scriptId` をキーにすることで、`file://` と `git://` で異なるキャッシュエントリが使用され、互いに干渉しなくなりました。

### 検証に使用したコード

参考までに、ログを仕込んだコードを掲載します。
...
createVirtualCode(scriptId, languageId, snapshot) {
    ...
    if (plugins.some(plugin => plugin.isValidFile?.(fileName, languageId))) {
        ...
        if (code) {
            ...
            console.log('[変更前] new code created, registering with key:', fileName);
...
createVirtualCode(scriptId, languageId, snapshot) {
    const fileName = asFileName(scriptId);
    if (plugins.some(plugin => plugin.isValidFile?.(fileName, languageId))) {
        // [変更後] String(scriptId) をキーとして使用
        const code = fileRegistry.get(String(scriptId));
        console.log('[変更後] createVirtualCode', {
            fileName,
            languageId,
            scheme: (scriptId as any).scheme ?? 'unknown',
        ...
        console.log('[変更後] cache hit:', !!code);
        if (code) {
            code.update(snapshot);
            return code;
        }
            ...
            console.log('[変更後] new code created, registering with key:', String(scriptId));
            fileRegistry.set(String(scriptId), code);
            return code;
        }
    }
